# -*- coding: utf-8 -*-
"""
Created on Tue Mar 23 18:09:21 2021
...
class ObjLoader:
    ...
    @staticmethod
    def array_to_vector3_array(array):
        # 1D-array를 3개씩 묶어서 vector3 array로 변경.

        v_num = int(array.size / 3)
        vector3_array = np.zeros(shape=(v_num, 3))
        for i in range(0, v_num):
            vector3_array[i] = [array[3*i], array[3*i + 1], array[3*i + 2]]
    ...
    def load_model(file, sorted=True):        
        # sorted = FALSE 이면 8개 한 세트로 내어놓는다. v, vt, vn.
        # sorted = TRUE 이면, v 먼저 쭉 나오고, vt 쭉 나오고, vn 쭉 나옴.
        ...
        return np.array(indices, dtype='uint32'), np.array(buffer, dtype='float32'), np.array(vert_coords, dtype='float32'), np.array(tex_coords, dtype='float32'), np.array(norm_coords, dtype='float32')


    @staticmethod
    def load_model_without_face(file, sorted=True):
        vert_coords = [] # will contain all the vertex coordinates
        tex_coords = [] # will contain all the texture coordinates
        norm_coords = [] # will contain all the vertex normals
